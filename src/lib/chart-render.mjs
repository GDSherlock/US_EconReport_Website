import {localizeChart,chartText} from '../data/chart-copy.en.mjs';
import { buildPanelGeometry, selectRange, curveDates } from './chart-geometry.mjs';
export const escapeHtml = value => String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
export const displayUnit = (unit,locale='zh') => locale==='en'?({'百万美元':'USD 100 million','万人':'10,000 people','万套':'10,000 homes','指数':'Index','月':'Months'}[unit]??chartText(unit,locale)):(unit==='百万美元'?'亿美元':unit);
...
function normalizePanel(panel,range,selectedPeriod) {
 if(panel.type==='yield-curve') {
  ...
  return {...panel,selectedPeriod:date,series:[{name:panel.title,frequency:'daily',points:panel.series.map((s,i)=>({period:`2000-01-${String(i+1).padStart(2,'0')}`,label:panel.tenors[i].label,value:s.points.find(p=>p.period===date)?.value??null}))}]};
 }
...
export function renderPanels(resolved, options={}) {
 const locale=options.locale??'zh', en=locale==='en';
 resolved=localizeChart(resolved,locale);
 ...
 return resolved.panels.map((original,index)=>{
  ...
  const svg=`<svg viewBox="0 0 ${g.width} ${g.height}" role="img" aria-labelledby="${id}-title ${id}-desc"><title id="${id}-title">${escapeHtml(panel.title)}</title><desc id="${id}-desc">${escapeHtml(panel.title)}${en?', unit: ':'，单位'}${displayUnit(panel.unit,locale)}${en?'. Full observations are available in the data table below.':'。完整读数见下方数据表。'}</desc><g class="chart-axis">${yTicks}${labels}</g>${marks}${g.empty?`<text x="${g.width/2}" y="130" text-anchor="middle">${en?'No observations available':'暂无可用观测'}</text>`:''}</svg>`;
  ...
  const latest=panel.series.map(s=>{const p=s.points.filter(p=>p.value!==null).at(-1);return p?`${s.name}${en?': ':'：'}${p.period}`:'';}).join(en?'; ':'；');
  ...
  const rows=tablePeriods.map(period=>`<tr><th scope="row">${escapeHtml(maps[0].get(period)?.label??period)}</th>${maps.map(m=>`<td>${formatValue(m.get(period)?.value??null,panel.unit,4)}${m.get(period)?.status==='provisional'?(en?' (provisional)':'（初步）'):''}</td>`).join('')}</tr>`).join('');
  return `<section class="chart-panel" data-panel="${index}"><div class="chart-panel-heading"><span>${escapeHtml(panel.title)}</span><span>${displayUnit(panel.unit,locale)}${panel.selectedPeriod?' · '+panel.selectedPeriod:''}</span></div><div class="chart-legend">${panel.series.map((s,i)=>`<span><i style="--series-color:${colors[i%colors.length]}"></i>${escapeHtml(s.name)}</span>`).join('')}</div>${plot}<p class="chart-readout" aria-live="polite">${panel.selectedPeriod?(en?'Observation date: ':'观测日期：')+panel.selectedPeriod:escapeHtml(latest)}</p><details class="chart-data-table"><summary>${en?'View data table':'查看数据表'}<span>${en?'Selected range':'当前区间'} · ${tablePeriods.length} ${en?'periods':'个期间'}</span></summary><div class="chart-table-scroll" tabindex="0" role="region" aria-label="${escapeHtml(panel.title)} ${en?'data table':'数据表'}"><table><caption>${escapeHtml(panel.title)}${en?' (':'（'}${displayUnit(panel.unit,locale)}${panel.selectedPeriod?(en?', ':'，')+panel.selectedPeriod:''}${en?')':'）'}</caption><thead><tr><th scope="col">${panel.type==='yield-curve'?(en?'Tenor':'期限'):(en?'Period':'统计期间')}</th>${panel.series.map(s=>`<th scope="col">${escapeHtml(s.name)}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table></div></details></section>`;
 }).join('');
}
export function renderChartFigure(resolved, options={}) {
 ...
 const controls=mode==='page'?(curve?`<label class="chart-date-label">${en?'Observation date':'观测日期'}<select data-curve-date aria-label="${en?'Yield curve observation date':'收益率曲线观测日期'}">${dates.slice().reverse().map(date=>`<option value="${date}">${date}</option>`).join('')}</select></label>`:`<div class="chart-ranges" role="group" aria-label="${escapeHtml(d.title)} ${en?'time range':'时间范围'}">${['1y','3y','5y','all'].map(r=>`<button type="button" data-range="${r}" aria-pressed="${r===range}">${(en?{'1y':'1Y','3y':'3Y','5y':'5Y',all:'All'}:{'1y':'1年','3y':'3年','5y':'5年',all:'全部'})[r]}</button>`).join('')}</div>`):'';
 ...
 const metadata=(resolved.metadata??[]).map(s=>`<li>${escapeHtml(s.name)}${en?': ':'：'}${(en?{daily:'Daily',weekly:'Weekly',monthly:'Monthly',quarterly:'Quarterly'}:{daily:'日度',weekly:'周度',monthly:'月度',quarterly:'季度'})[s.frequency]}${en?', ':'，'}${escapeHtml(en?({'百万美元':'USD million'}[s.unit]??displayUnit(s.unit,locale)):s.unit)}${en?', ':'，'}${(en?{sa:'Seasonally adjusted',nsa:'Not seasonally adjusted',unknown:'Seasonal adjustment not specified by source'}:{sa:'季调',nsa:'未季调',unknown:'来源未明确季调状态'})[s.adjustment]}${en?'. ':'。'}${escapeHtml(s.measure)}</li>`).join('');
 return `<figure class="macro-chart" id="${escapeHtml(id)}" data-mode="${mode}" data-locale="${locale}" data-release="${escapeHtml(resolved.releaseId)}" data-chart-id="${escapeHtml(d.chartId)}"><figcaption class="chart-title">${escapeHtml(d.title)}</figcaption>${options.caption?`<p class="chart-caption">${escapeHtml(options.caption)}</p>`:''}${controls}<div class="chart-panels">${renderPanels(resolved,{...options,range,instanceId:id})}</div><p class="chart-method">${escapeHtml(d.note??'')}</p><p class="chart-source">${en?'Sources: ':'来源：'}${escapeHtml(en?'Wind, Bloomberg':sourceLabels)}<br/>${en?'Data updated: ':'数据更新日期：'}${escapeHtml(resolved.sourceEdition)} · ${escapeHtml(options.versionNote??(en?'Latest imported release; reference periods vary by series.':'展示最新已导入版本；统计期间以各序列为准。'))}</p><details class="chart-source-detail"><summary>${en?'Methodology':'指标口径'}</summary><ul>${metadata}</ul></details>${options.detailHref?`<a class="chart-detail-link" href="${escapeHtml(options.detailHref)}">${en?'View latest data and full history':'查看最新数据与完整历史'} <span aria-hidden="true">↗</span></a>`:''}${mode==='page'?`<script type="application/json" class="chart-payload">${safe}</script>`:''}</figure>`;
}
